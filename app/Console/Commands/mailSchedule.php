...
class mailSchedule extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($orders as $order) {
            ...
            // date calc
            $datetime1 = new DateTime($order['givedDate']);
            $datetime2 = new \DateTime('NOW');
            $days = $datetime1->diff($datetime2)->format('%a');
            // date returt date
            $returnDate = new DateTime($order['givedDate']);
            ...
            if ($order->extend == 1) {
                if ($days >= 8) {
                    ...
                    <br>Хамрагдах хугацаа /Period/ :" . $order["givedDate"] . " - " . $returnDate->format('Y-m-d H:i:s') . "
