...
<div id="accordion" class="text-white">
  ...
  ?>
    ...
    @for ($i = 0; $i < count($shops); $i++) {{-- {{$Numbers[1]}} --}} <div class="card bg-dark">
        <div class="card-header">
            <h5 class="mb-0">
                <button class="btn btn-link" data-toggle="collapse" data-target="#collapse{{$Numbers[$i]}}"
                    aria-controls="collapse{{$Numbers[$i]}}">
        ...
        <div id="collapse{{$Numbers[$i]}}" class="collapse hide" aria-labelledby="headingOne" data-parent="#accordion">
          ...
          ?>
                ...
                <input type="hidden" class="form-control" name="customerId" value="{{session('LoggedCustomer')}}"
                    readonly>
